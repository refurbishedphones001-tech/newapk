...
(() => {
    ...
    // Sizes tried (largest first) when handing a frame to the native/native-like
    // inference path. A wider ladder means devices under memory pressure, or
    // photos in an awkward format, still get a usable pass instead of failing
    // outright on the first size.
    const NATIVE_SIDE_LADDER = [1400, 1000, 700, 480];

    // Does one full attempt at decoding + grading `sample`. Throws on failure;
    // resolves with the fields to copy onto the sample on success. Kept
    // separate from `analyze()` so the whole pipeline can be retried cleanly.
    async function runAnalysisAttempt(sample) {
        const started = performance.now();
        const bitmap = await decodeImage(sample.file);

        // A thumbnail is a nice-to-have, not part of the actual grading — never
        // let a thumbnail-rendering hiccup turn a successful analysis into a
        // reported failure.
        let thumb = '';
        try { thumb = makeThumbnail(bitmap); } catch (err) { console.warn('Thumbnail failed:', err); }
        ...
        try {
            if (NATIVE_ANDROID) {
                ...
                // Work down through smaller sizes before giving up on this attempt.
                for (const maxSide of NATIVE_SIDE_LADDER) {
                    try {
                ...
                ms = Number.isFinite(result.ms) ? Math.round(result.ms) : Math.round(performance.now() - startedNative);
            } else {
        ...
        return {
            ...
            low: probs[best] < CONFIG.lowConfidence,
            ms: ms || Math.round(performance.now() - started)
        };
    }
    ...
    async function analyze(sample) {
        ...
        try {
            ...
            // Most failures at this point are transient (a slow frame, a brief
            // memory squeeze, a one-off decode hiccup) and a fresh attempt just
            // works — so retry the whole pipeline once before telling the user
            // anything went wrong.
            let outcome = null;
            let lastError = null;
            for (let attempt = 0; attempt < 2; attempt++) {
                try {
                    outcome = await runAnalysisAttempt(sample);
                    break;
                } catch (err) {
                    lastError = err;
            ...
            if (!outcome) throw (lastError || new Error('Analysis failed'));

            sample.thumb = outcome.thumb;
            sample.className = outcome.className;
            sample.classKey = outcome.classKey;
            sample.probs = outcome.probs;
            sample.confidence = outcome.confidence;
            sample.low = outcome.low;
            sample.ms = outcome.ms;
            sample.status = 'done';
    ...
    // Some Android WebViews are slow to hand over the first frame while the
    // camera HAL warms up, so this is generous rather than tight.
    const CAMERA_READY_TIMEOUT_MS = 7000;

    // Ask for the back camera first, but don't let a device that can't satisfy
    // the "ideal" hints (resolution, exact facing mode) fail outright — fall
    // back to whatever camera is available.
    async function openCameraStream() {
        try {
            ...
            if (error && (error.name === 'OverconstrainedError' || error.name === 'NotFoundError')) {
                // Retry with minimal constraints so a device with only one
                // camera, or one that rejects the resolution hints, still works.
