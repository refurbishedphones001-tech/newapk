...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Camera permission is requested lazily, from onPermissionRequest below,
        // the moment the page actually asks for it (i.e. when the user taps
        // "Start camera"). Asking for it up front at launch — before the user
        // has any context for the prompt — makes an accidental "Deny" far more
        // likely, and on most OEM builds a second request after that is then
        // silently auto-denied with no dialog at all, permanently breaking the
        // live camera for that install.
    ...
    public void onRequestPermissionsResult(int requestCode, String[] permissions, int[] grantResults) {
        ...
        if (requestCode == CAMERA_REQ && pendingPermissionRequest != null) {
            ...
            if (granted) {
                ...
                // "Denied, and don't ask again" (or an OEM policy) means no dialog
                // will ever show up if the user taps "Start camera" again. Point
                // them at the app's permission settings instead of failing quietly.
                if (ContextCompat.checkSelfPermission(this, Manifest.permission.CAMERA) != PackageManager.PERMISSION_GRANTED
                        && !shouldShowRequestPermissionRationale(Manifest.permission.CAMERA)) {
                    Toast.makeText(this,
                            "Camera access is off for OnionQualityAI. Enable it in Settings \u2192 Apps \u2192 OnionQualityAI \u2192 Permissions to use live scanning.",
